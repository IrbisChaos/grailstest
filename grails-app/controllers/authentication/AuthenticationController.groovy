...
class AuthenticationController {
	...
	def signup = { SignupForm form ->
		...
		if (!form.hasErrors()) {
			...
			if ((signupResult.result == 0) || (signupResult.result == AuthenticatedUser.ERROR.AWAITING_CONFIRMATION.val())) {
				if (log.debugEnabled) {
					if (signupResult == AuthenticatedUser.ERROR.AWAITING_CONFIRMATION.val()) {

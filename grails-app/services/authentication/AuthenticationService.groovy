...
import static authentication.AuthenticatedUser.ERROR.LOGIN_NAME_NOT_AVAILABLE
import static authentication.AuthenticatedUser.ERROR.NO_SUCH_LOGIN
import static authentication.AuthenticatedUser.ERROR.INCORRECT_CREDENTIALS
import static authentication.AuthenticatedUser.ERROR.AWAITING_CONFIRMATION
...
class AuthenticationService {
	...
	static enum STATUS implements EnumBase{
		NEW(0),
		VALID(1),
		AWAITING_CONFIRMATION(2),
		CONFIRMATION_LAPSED(3)
		STATUS(int value) {
			this.value = value
		}
     ...
     * If so it will return a user object with the LOGIN_NAME_NOT_AVAILABLE result.</p>
...
//        assert !configurationHolder.config.authentication?.signup?.disabled, "Cannot perform signup, it is disabled in Config"
		...
		if (user) {
			authUser.result = LOGIN_NAME_NOT_AVAILABLE.val()
			return authUser
		}
		...
		user.email = email
		user.status = STATUS.AWAITING_CONFIRMATION.val()
		...
		if ((authUser.result == 0) || (authUser.result == AWAITING_CONFIRMATION.val())) {
	...
	AuthenticatedUser login(login, pass) {
		def user = fireEvent('FindByLogin', login)
		userCur = user
		Membership member =  Membership.withCriteria {
			eq('login', login)
		}?.get(0)
//		actor.subj = member?.get(0)?.org == null ? member?.get(0)?.prsn : member?.get(0)?.org
//		log.error("member:")
//		log.error(member)
//		log.error("member.org:")
//		log.error(member?.org)
//		log.error("member.prsn:")
//		log.error(member?.prsn)
		actor.subj = member.org == null ? member.prsn : member.org
		def token = new AuthenticatedUser(login:login, loginTime:new Date())

		if (!user) {
			token.result = NO_SUCH_LOGIN.val()
            setSessionUser(null)		    
		} else if (user.password != encodePassword(pass)) {
			token.result = INCORRECT_CREDENTIALS.val()
            setSessionUser(null)		    
		} else {
	...
	protected userStatusToResult(def userStatus) {
		...
		switch (userStatus) {
			case STATUS.NEW.val() :
			case STATUS.VALID.val() :
				value = 0
				break
			case STATUS.AWAITING_CONFIRMATION.val() :
			case STATUS.CONFIRMATION_LAPSED.val() :
				value = AWAITING_CONFIRMATION.val()
				break
			default :
	...
	*/
    private static defaultEventHandlers = [
        ...
        onFindByLoginByStatusValid:{ loginID -> AuthenticationService.userDomainClass.findAllByLoginAndStatus(loginID, STATUS.VALID.val()) },
